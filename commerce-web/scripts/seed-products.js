 ...
 * Run this after starting the dev server to populate the database with sample products
 * 
 * Usage: npm run seed (after dev server is running)
 */
...
async function seedDatabase() {
  ...
  for (const product of sampleProducts) {
    try {
      ...
      if (response.ok) {
        ...
        console.error(`❌ Failed to create ${product.name}: ${response.statusText}`);
        errorCount++;
      }
    } catch (error) {
  ...
  console.log(`\n🎉 Visit http://localhost:3000/products to see your products!`);
